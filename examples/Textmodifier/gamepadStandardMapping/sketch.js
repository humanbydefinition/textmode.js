...
labelLayer.draw(() => {
	...
	drawText('TEXTMODIFIER.GAMEPADSTANDARDMAPPING', x, y++, 100, 255, 140);
	drawText('------------------------------------', x, y++, 80, 100, 150);
	drawText('CONCEPT: GAMEPAD INPUT', x, y++, 100, 220, 255);
	drawText('Works with browser pads.', x, y++, 140, 160, 190);
	drawText('------------------------------------', x, y++, 80, 100, 150);
	const pad = t.gamepads[0];
	const state = pad && pad.standard ? 'YES' : 'NO';
	drawText(`STANDARD: ${state}`, x, y++, 140, 255, 180);
});
